...
import numpy as np
import pytest
from src.strategies.statistical.gaussian_nb_strategy import GaussianNBStrategy
...
def test_data_source_is_features():
    assert GaussianNBStrategy.data_source == "features"


def test_no_nan_in_output(sample_df):
    s = GaussianNBStrategy()
    s.fit(sample_df)
    result = s.predict(sample_df)

...
**Task 6, Step 1 was not run as-written.** The two documented commands (`python scripts/train_models.py --market china` and `python scripts/precompute_dashboard.py --market china`) were attempted during original execution, but the second one crashed mid-run three times while computing live signals for all 500 China tickers, with a Rust/Polars allocator panic (`memory allocation of 15803824 bytes failed`). `train_models.py --market china` and `precompute_dashboard.py`'s first three steps (data summary, backtests, leaderboard) had already completed successfully in an earlier attempt and were reused as-is. The live-signals step was instead produced by a since-deleted one-off script (`scratch_china_signals_one_by_one.py`) that processed the 500 tickers one at a time rather than via the documented `step_signals()` codepath, to bound peak memory. No source files were changed to work around this (out of scope for Task 6) — it was a runtime workaround only, and the resulting `markets/china/data/cache/signals.json` (16,000 signals, 500/500 tickers) is valid, just produced with weaker provenance than the plan intended.

**Re-verification (final-review fix wave, 2026-08-01):** a whole-branch reviewer measured `build_live_features()`'s all-tickers-then-concat pattern in isolation at ~694MB peak RSS — far below what should OOM, and less memory than the US market's identical pattern handles successfully in production over 77% more rows — casting doubt on the original crash's stated root cause. To settle it, `step_signals()` was re-invoked directly (not the full CLI) with zero other processes running (confirmed via `Get-Process python` beforehand). It **crashed again**, reproducing the exact same error signature (`memory allocation of 15803824 bytes failed`) with no competing process present, which rules out "an orphaned background process was consuming memory" as the explanation for at least this occurrence. The captured log stopped after the initial `[4/4] live signals` line with no per-strategy log lines visible, weakly suggesting the failure is in `build_live_features()` and/or the `ticker_pdfs` per-ticker partitioning loop that immediately follows it in `step_signals()`, before the first strategy iteration — but output buffering on the redirected log means this isn't conclusive. Net result: the crash is real and reproducible under clean conditions, but the exact culprit line is still not pinned down, and `python scripts/precompute_dashboard.py --market china` should **not** be assumed safely re-runnable. A follow-up plan to profile and fix `build_live_features()`/`step_signals()`'s memory usage at 500-ticker scale is needed; it was out of scope for this fix wave (diagnosis and housekeeping only, no source changes).
